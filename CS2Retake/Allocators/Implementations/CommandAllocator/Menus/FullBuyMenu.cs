...
namespace CS2Retake.Allocators.Implementations.CommandAllocator.Menus
{
    public class FullBuyMenu
    {
        ...
        public void OpenAWPChanceMenu(CCSPlayerController player, CsTeam team)
        {
            ...
            var teamString = team == CsTeam.CounterTerrorist ? "CT" : "T";
            var awpChance = CacheManager.Instance.GetFullBuyWeapons(player, team).awpChance ?? 0;
            var isEnabled = awpChance > 0;
            var optionLabel = isEnabled ? $"{teamString}_Disable AWP" : $"{teamString}_Enable AWP";

            menu.AddMenuOption(optionLabel, OnSelectAWPChance);
        ...
        private static void OnSelectAWPChance(CCSPlayerController player, ChatMenuOption chatMenuOption)
        {
            ...
            var enabledChance = GetEnabledAwpChance(team);
            var chance = currentChance > 0 ? 0 : enabledChance;

            MessageUtils.PrintToPlayerOrServer(BuildAwpToggleMessage(chance > 0, enabledChance), player);
        ...
        private static int GetEnabledAwpChance(CsTeam team)
        {
            var awpSettings = team == CsTeam.CounterTerrorist ? _config.AWPChanceCT : _config.AWPChanceT;

            return awpSettings.Chances.OrderDescending().FirstOrDefault();
        }

        private static string BuildAwpToggleMessage(bool enabled, int chance)
        {
